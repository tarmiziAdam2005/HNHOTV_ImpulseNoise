function out = HNHOTV_OGS_Impulse(f,Img,K,opts)

%This program solves the proposed Hybrid Non-convex Higher order 
%Overlapping group sparse Total variation image restoration for Salt and 
%Pepper impulse noise and random valued impulse noise

% For more details of this code please refer to our latest article

% "Combined higher order non-convex total variation with overlapping 
%    group sparsity for impulse noise removal" published in Multimedia
%    Tools and Applications, Springer


% If you happen to use this code for research purpose, please cite our
% paper accordingly. Thank you.

%Updated on 19/1/2018 by Tarmizi Adam
% Release date to public: 19 February 2021
...
tg = tic;
    for k = 1:Nit
        ...
        w1      =shrink(x1,wgt1.*omega/rho_w);
        w2      =shrink(x2,wgt2.*omega/rho_w);
        w3      =shrink(x3,wgt3.*omega/rho_w);
        w4      =shrink(x4,wgt4.*omega/rho_w);
            
        %********* r sub-problem ***********************
        
        tmp_r = q + mu_r/rho_r;
        wgt_r = (lam/rho_r)*p_r./(abs(q)+ 0.00001).^(1-(p_r));
        
        r = shrink(tmp_r, wgt_r.*lam/rho_r);
       
        %********* u sub-problem (Least squares problem, use FFT's)********
        u_old   = u;
        temp_u = r + f - mu_r/rho_r;
 ...
 out.snrRes              = snr_fun(u, double(Img));
